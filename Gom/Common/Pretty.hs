...
instance Pretty Ctor where
  pretty (Simple n fs) = 
     text (show n) <> parens (hsep $ punctuate (char ',') prettyfs)
     where prettyfs = map (\(x,t) -> text (show x) <> 
                      char ':' <> text (show t)) fs
  pretty (Variadic n t) = text (show n) <> parens (text (show t) <> char '*')

instance Pretty SortDef where
  pretty (SortDef n mcn cs) = 
    text (show n) <> prettycn mcn <> align (char '=' <+> prettycs)
    where prettycs = fillSep $ withPipes (map pretty cs)  
          prettycn Nothing   = empty
          prettycn (Just cn) = space <> text "implemented by" <+> 
                               pretty cn <> space
